...
class Encoder(nn.Module):
    def __init__(self, latent_dim, num_classes):
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(3, 64, 4, 2, 1), nn.ReLU(),             # 128x128
            nn.Conv2d(64, 128, 4, 2, 1), nn.BatchNorm2d(128), nn.ReLU(),  # 64x64
            nn.Conv2d(128, 256, 4, 2, 1), nn.BatchNorm2d(256), nn.ReLU(),  # 32x32
            nn.Conv2d(256, 512, 4, 2, 1), nn.BatchNorm2d(512), nn.ReLU(),  # 16x16
        )
        self.fc_mu     = nn.Linear(512*16*16 + num_classes, latent_dim)
        self.fc_logvar = nn.Linear(512*16*16 + num_classes, latent_dim)

    def forward(self, x, labels):
        x = self.conv(x)                          # [B,512,16,16]
        x = x.view(x.size(0), -1)                 # [B,512*16*16]
        x = torch.cat([x, labels], dim=1)         # [B,512*16*16+num_classes]
        return self.fc_mu(x), self.fc_logvar(x)   # two tensors [B, latent_dim]
...
class Generator(nn.Module):
    def __init__(self, latent_dim, num_classes):
        ...
        self.deconv = nn.Sequential(
            nn.ConvTranspose2d(512, 256, 4, 2, 1), nn.BatchNorm2d(256), nn.ReLU(),  # 32x32
            nn.ConvTranspose2d(256, 128, 4, 2, 1), nn.BatchNorm2d(128), nn.ReLU(),  # 64x64
            nn.ConvTranspose2d(128, 64, 4, 2, 1),  nn.BatchNorm2d(64),  nn.ReLU(),  # 128x128
            nn.ConvTranspose2d(64, 3, 4, 2, 1),    nn.Tanh()                      # 256x256
        )

    def forward(self, z, labels):
        x = torch.cat([z, labels], dim=1)        # [B, latent_dim+num_classes]
        x = self.fc(x).view(-1, 512, 16, 16)      # [B,512,16,16]
        return self.deconv(x)                    # [B,3,256,256]
...
class Discriminator(nn.Module):
    ...
    def forward(self, x):
        x = self.conv(x).flatten(start_dim=1)    # [B,512*16*16]
...
def train_cvaegan(models, dataloader, num_epochs, device, num_classes, latent_dim):
    ...
    for epoch in range(1, num_epochs+1):
        ep_recon, ep_kl, ep_adv, ep_cls, ep_fm, ep_gen, ep_disc = (0,) * 7
        batches = 0
        ...
        for imgs, labels in dataloader:
            ...
            real_val = discriminator(imgs)
            fake_val = discriminator(fake_imgs.detach())
            ...
            fake_val2 = discriminator(fake_imgs)
            fake_cls2 = classifier(fake_imgs)

            f_real_D = discriminator.conv(imgs)
            f_fake_D = discriminator.conv(fake_imgs)
            ...
            kl_l = kl_divergence(mu, logvar) / batch_size
            kl_l = torch.clamp(kl_l, max=1.0) 
        ...
        print(f"Epoch {epoch}/{num_epochs} | D: {disc_list[-1]:.4f} | G: {gen_list[-1]:.4f}")
